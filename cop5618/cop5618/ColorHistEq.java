...
public class ColorHistEq {
	...
	static Timer colorHistEq_serial(BufferedImage image, BufferedImage newImage) {
		...
		Map<Object, java.util.List<HSBPixel>> binsMap = Arrays.stream(sourceRGBPixelArray) // get
																							// array
																							// of
																							// pixels
																							// from
				...
				// get histograms bins map with list of pixels in each bin
		...
		double[] binsArray = binsMap.entrySet().stream().map(Map.Entry::getValue).mapToDouble(hsbList -> hsbList.size())
				.toArray();
		...
		int[] outputRGBPixelArray = Arrays.stream(sourceRGBPixelArray).mapToObj(pixel -> {
			...
			if (hsb.b <= 0.1) {
				hsb.b = (float) binsCPArray[0];
			} else if (hsb.b <= 0.2) {
				hsb.b = (float) binsCPArray[1];
			} else if (hsb.b <= 0.3) {
				hsb.b = (float) binsCPArray[2];
			} else if (hsb.b <= 0.4) {
				hsb.b = (float) binsCPArray[3];
			} else if (hsb.b <= 0.5) {
				hsb.b = (float) binsCPArray[4];
			} else if (hsb.b <= 0.6) {
				hsb.b = (float) binsCPArray[5];
			} else if (hsb.b <= 0.7) {
				hsb.b = (float) binsCPArray[6];
			} else if (hsb.b <= 0.8) {
				hsb.b = (float) binsCPArray[7];
			} else if (hsb.b <= 0.9) {
				hsb.b = (float) binsCPArray[8];
			} else {
				hsb.b = (float) binsCPArray[9];
			}
			return hsb;
		}).mapToInt(hsb -> java.awt.Color.HSBtoRGB(hsb.h, hsb.s, hsb.b)).toArray();
	...
	static Timer colorHistEq_parallel(FJBufferedImage image, FJBufferedImage newImage) {
		...
		Map<Object, java.util.List<HSBPixel>> binsMap = Arrays.stream(sourceRGBPixelArray).parallel() // get
				// array
				// of
				// pixels
				// from
